...
def test_conversion(convert2rhel):
    activation_key = f'[subscription_manager]\nactivation_key = {env.str("RHSM_KEY")}\norg = {env.str("RHSM_ORG")}'
    config = [Config("~/.convert2rhel.ini", activation_key)]
    create_files(config)

    with convert2rhel(f'-y --no-rpm-va --serverurl {env.str("RHSM_SERVER_URL")} --debug') as c2r:
